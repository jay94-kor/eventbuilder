...
export default function AuthProvider({ children }: AuthProviderProps) {
  ...
  // useRef를 사용하여 인터셉터 ID를 저장
  const interceptorIds = useRef<{ request?: number; response?: number }>({});
  ...
  useEffect(() => {
    // 이전 인터셉터가 있다면 제거 (새로운 인터셉터 등록 전)
    if (interceptorIds.current.request !== undefined) {
      api.interceptors.request.eject(interceptorIds.current.request);
    }
    ...
    interceptorIds.current.request = requestInterceptorId; // ID 저장
    ...
    return () => {
      if (interceptorIds.current.request !== undefined) {
        api.interceptors.request.eject(interceptorIds.current.request);
        interceptorIds.current.request = undefined; // ID 초기화
      }
      if (interceptorIds.current.response !== undefined) {
        api.interceptors.response.eject(interceptorIds.current.response);
